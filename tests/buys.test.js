...
describe("GET /buys/sales -> get user sales", () => {
  it("it should return 401 status code -> not authorized", async () => {
    const response = await request(app).get("/buys/sales");
    expect(response.status).toBe(401);
    expect(response.body.msg).toBe("You are not authorized! Please login...");
  });
  it("it should return 200 status code -> no admin user logged in", async () => {
  ...
  it("it should return 200 status code -> get user sales", async () => {
    const response = await request(app)
      .get("/buys/sales")
      .set("Cookie", cookie);
    expect(response.status).toBe(200);
...
describe("GET /buys/publication/:id route -> get publication buys", () => {
  ...
  it("it should return 404 status code -> publication no buys", async () => {
    const response = await request(app)
      .get(`/buys/publication/${publication1_id}`)
      .set("Cookie", cookie);
    expect(response.status).toBe(404);
    expect(response.body.msg).toBe(
